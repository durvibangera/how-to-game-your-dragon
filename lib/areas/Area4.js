...
export class Area4 extends AreaBase {
  ...
  populate() {
    ...
    // Cloud clusters - volumetric-ish, at flight altitude
    for (let i = 0; i < 15; i++) {
    ...
    // Wispy pass-through clouds right along the flight corridor
    for (let i = 0; i < 8; i++) {
      const wisp = this._createWispCloud();
      wisp.position.set(
        (Math.random() - 0.5) * 30,
        15 + Math.random() * 15,
        this.baseZ - 10 - i * 14
      );
      wisp.scale.setScalar(1.5 + Math.random() * 2);
      this.group.add(wisp);
      this.clouds.push(wisp);
    }
  ...
  _createCloud() {
    ...
    for (let i = 0; i < puffs; i++) {
      const puff = new THREE.Mesh(
        new THREE.SphereGeometry(2 + Math.random() * 2.5, 5, 3),
        mat
      );
      puff.position.set(
        (Math.random() - 0.5) * 5,
  ...
  _createWispCloud() {
    ...
    // Flat wide ellipsoids - cheap wispy feel
    for (let i = 0; i < 2; i++) {
      const wisp = new THREE.Mesh(
        new THREE.SphereGeometry(3 + Math.random() * 3, 5, 3),
        mat
      );
      wisp.scale.set(1.5, 0.25, 1);
      wisp.position.set(
        (Math.random() - 0.5) * 4,
        Math.random() * 0.5,
        (Math.random() - 0.5) * 3
