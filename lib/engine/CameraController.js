...
export class CameraController {
  ...
  update(progress, delta) {
    ...
    const tLook = Math.min(t + 0.003, 0.999);
    ...
    // Look-at: slightly ahead of dragon center
    ...
    // Subtle banking roll effect on turns
    if (t > 0.005 && t < 0.995) {
      const tangent = this.curve.getTangentAt(t);
      const tangentBefore = this.curve.getTangentAt(Math.max(t - 0.005, 0));
      const cross = new THREE.Vector3().crossVectors(tangentBefore, tangent);
      const roll = THREE.MathUtils.clamp(cross.y * 6, -0.15, 0.15);
