...
/**
 * Builds the dragon flight path as a continuous spline through 6 HTTYD months + epilogue.
...
export class TrackBuilder {
  ...
  _generateTrackPoints() {
    const points = [];
    const monthLength = 120;
    const totalMonths = 6;

    for (let m = 0; m < totalMonths; m++) {
      const baseZ = -m * monthLength;
      const baseX = Math.sin(m * 0.5) * 40;
      const pts = this._getMonthPoints(m, baseX, baseZ, monthLength);
      points.push(...pts);
    }

    // EPILOGUE TRACK
    const epilogueStart = -totalMonths * monthLength;
    const epilogueLength = 300;
    const epilogueSegments = 20;
    const lastMonthBaseX = Math.sin(5 * 0.5) * 40;

    for (let i = 1; i <= epilogueSegments; i++) {
      const t = i / epilogueSegments;
      const easeOut = 1 - Math.pow(1 - t, 3);
      const z = epilogueStart - t * epilogueLength;
      const x = lastMonthBaseX * (1 - easeOut * 0.8) + Math.sin(t * Math.PI * 0.3) * 5;
  ...
  _getMonthPoints(monthIndex, baseX, baseZ, len) {
    const pts = [];
    const segments = 16;
