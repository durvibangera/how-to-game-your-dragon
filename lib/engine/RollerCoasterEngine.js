...
export class RollerCoasterEngine {
  ...
  _onKeyDown(e) {
    ...
    if (['Space', 'KeyC', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.code)) {
      e.preventDefault();
    }
  ...
  _navigateToChapter(chapterId) {
    if (chapterId.startsWith('area-')) {
      ...
      this.siegeGame = new SiegeGame(siegeContainer, (success) => {
        ...
        if (success) {
          window.location.href = '/final-game';
        } else {
  ...
  _showControlsHint() {
    ...
    `;
    hint.innerHTML = `
      <div style="margin-bottom:3px;"><b>A</b>/<b>D</b> Turn &nbsp;·&nbsp; <b>Space</b> Climb &nbsp;·&nbsp; <b>C</b> Dive</div>
  ...
  _updateFlightInput(delta) {
    ...
    // ─── Pitch: Space = climb, C = dive ───
    let pitchInput = 0;
    if (k['Space'])                                pitchInput += 1;
    if (k['KeyC'])                                 pitchInput -= 1;
