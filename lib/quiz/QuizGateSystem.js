 ...
 * Creates floating fire hoops at area boundaries that the dragon flies through.
 * Purely visual — no quiz or game logic.
 */
export class GateSystem {
  constructor(scene, ui, particleSystem, trackCurve, areaTrackEnd) {
    this.scene = scene;
    this.ui = ui;
    this.particleSystem = particleSystem;
    this.trackCurve = trackCurve;
    this.areaTrackEnd = areaTrackEnd;
  ...
  _createGates() {
    for (let i = 0; i < 5; i++) {
      ...
      // Position gate ON the track curve at the area boundary
      const t = Math.min(((i + 1) / 6) * this.areaTrackEnd, 0.998);
