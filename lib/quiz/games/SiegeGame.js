...
export class SiegeGame {
  ...
  _spawnWave(waveNum) {
    ...
    const waveNames = [
      '', 
      'WAVE 1 — DRAGON SCOUTS', 
      'WAVE 2 — THE AWAKENING',
      'WAVE 3 — DRAGON HORDE',
      'WAVE 4 — DARK LEGIONS',
      'WAVE 5 — FIRE & FURY',
      'WAVE 6 — WRATH OF ODINN',
      'WAVE 7 — RAGNAROK'
    ];
    ...
    switch (waveNum) {
      case 1: // Dragon Scouts: Basic enemies
        spawnWithEffect(spawnPoints[0].x, spawnPoints[0].y, 'slime', 15, 3);
        spawnWithEffect(spawnPoints[1].x, spawnPoints[1].y, 'slime', 15, 3);
        spawnWithEffect(spawnPoints[4].x, spawnPoints[4].y, 'knight', 25, 4);
        break;

      case 2: // The Awakening: More basic enemies
        spawnWithEffect(spawnPoints[0].x, spawnPoints[0].y, 'knight', 28, 4);
        spawnWithEffect(spawnPoints[1].x, spawnPoints[1].y, 'knight', 28, 4);
        spawnWithEffect(spawnPoints[2].x, spawnPoints[2].y, 'slime', 18, 3);
        spawnWithEffect(spawnPoints[4].x, spawnPoints[4].y, 'archer', 18, 5);
        break;

      case 3: // Dragon Horde: Introducing shields and ranged
        spawnWithEffect(spawnPoints[0].x, spawnPoints[0].y, 'shield_guard', 30, 4);
        spawnWithEffect(spawnPoints[1].x, spawnPoints[1].y, 'knight', 30, 5);
        spawnWithEffect(spawnPoints[2].x, spawnPoints[2].y, 'archer', 20, 5);
        spawnWithEffect(spawnPoints[4].x, spawnPoints[4].y, 'crossbow_archer', 22, 6);
        spawnWithEffect(spawnPoints[5].x, spawnPoints[5].y, 'slime', 18, 3);
        break;

      case 4: // Dark Legions: Elite enemies begin
        spawnWithEffect(spawnPoints[0].x, spawnPoints[0].y, 'shield_guard', 35, 5);
        spawnWithEffect(spawnPoints[1].x, spawnPoints[1].y, 'shield_guard', 35, 5);
        spawnWithEffect(spawnPoints[2].x, spawnPoints[2].y, 'berserker', 28, 8);
        spawnWithEffect(spawnPoints[3].x, spawnPoints[3].y, 'fire_mage', 35, 10);
        spawnWithEffect(spawnPoints[4].x, spawnPoints[4].y, 'crossbow_archer', 25, 7);
        this.lavaPoolMaxActive = 2;
        break;

      case 5: // Fire & Fury: More elite enemies
        spawnWithEffect(spawnPoints[0].x, spawnPoints[0].y, 'berserker', 30, 9);
        spawnWithEffect(spawnPoints[1].x, spawnPoints[1].y, 'dark_knight', 60, 12);
        spawnWithEffect(spawnPoints[2].x, spawnPoints[2].y, 'fire_mage', 38, 11);
        spawnWithEffect(spawnPoints[3].x, spawnPoints[3].y, 'necromancer', 35, 8);
        spawnWithEffect(spawnPoints[5].x, spawnPoints[5].y, 'shield_guard', 40, 6);
        spawnWithEffect(spawnPoints[6].x, spawnPoints[6].y, 'crossbow_archer', 28, 8);
        this.lavaPoolMaxActive = 3;
        this._showHudMessage('🔥 The dragon fire intensifies! Watch for lava pools! 🔥');
        break;

      case 6: // Wrath of Odinn: Very challenging
        spawnWithEffect(spawnPoints[0].x, spawnPoints[0].y, 'war_golem', 90, 14);
        spawnWithEffect(spawnPoints[1].x, spawnPoints[1].y, 'dark_knight', 70, 14);
        spawnWithEffect(spawnPoints[2].x, spawnPoints[2].y, 'berserker', 32, 9);
        spawnWithEffect(spawnPoints[3].x, spawnPoints[3].y, 'fire_mage', 42, 12);
        spawnWithEffect(spawnPoints[4].x, spawnPoints[4].y, 'necromancer', 38, 8);
        spawnWithEffect(spawnPoints[5].x, spawnPoints[5].y, 'shield_guard', 45, 7);
        spawnWithEffect(spawnPoints[6].x, spawnPoints[6].y, 'crossbow_archer', 30, 9);
        this.lavaPoolMaxActive = 4;
        this.shakeAmount = 8;
        this.shakeTimer = 25;
        this._showHudMessage('⚡ Odinn\'s wrath descends! Steel yourself, Viking! ⚡');
        break;

      case 7: // RAGNAROK: Final wave - captain + war_golem + elite mix, max lava
    ...
    if (this.waveState === 'pause') {
      ...
      if (alive === 0) {
        ...
        if (this.currentWave >= this.totalWaves) {
          ...
          // Grant halberd after wave 3 is cleared
          if (this.currentWave === 3 && !this.player.hasWeapons.halberd) {
            this.player.hasWeapons.halberd = true;
            this.player.weapon = 'halberd';
            this._showWeaponPickup('halberd');
            this._showHudMessage('⚔️ RUNIC HALBERD unlocked! Wide sweeping strikes pierce shields!');
          }
